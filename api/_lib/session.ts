...
export type Session = { handle: string; iat: number }

const SESSION_COOKIE = 'session'
const SESSION_MAX_AGE = 60 * 60 * 24 * 30 // 30 days

export async function createSessionCookie(handle: string): Promise<string> {
  const token = await signValue(requireSecret(), { handle, iat: Date.now() } satisfies Session)
